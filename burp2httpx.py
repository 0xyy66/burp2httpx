...
parser.add_argument('-r', '--requests', action='store_true', help='Generate requests module code')
...
def gen_httpx_code(url, method, headers, data):
        headers_str = '{\n'
...
'''
        if data:
                ...
                code += f'''
\tdata = {data_str}
\tres = client.{method.lower()}('{url}', data=data)
\tprint(res.status_code)
'''
        else:
                code += f'''
\tres = client.{method.lower()}('{url}')
\tprint(res.status_code)
'''
        return code

def gen_requests_code(url, method, headers, data):
        headers_str = '{\n'
        for k,v in headers.items():
                headers_str += f'\t\t"{k}": "{v}",\n'
        headers_str += '\t}'
...
if __name__ == "__main__":
        ...
        if args.requests:
                code = gen_requests_code(url, method, headers, data)
        else:
                code = gen_httpx_code(url, method, headers, data)
        outputfile = args.output[0] if args.output else False
        if outputfile:
